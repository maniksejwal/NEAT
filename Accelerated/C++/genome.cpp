...
Genome::Genome(int id, std::vector<Trait *> t, std::vector<NNode *> n, std::vector<Link *> links) {
    ...
    for (curlink = links.begin(); curlink != links.end(); ++curlink) {
        ...
        tempgene = new Gene((*curlink)->linktrait, (*curlink)->weight, (*curlink)->in_node, (*curlink)->out_node,
...
Genome::Genome(int id, std::ifstream &iFile) {
    ...
    while (!done) {
        ...
        //printf(curword);
        //printf(" test\n");
        if (strcmp(curword, "genomeend") == 0) {                //Check for end of Genome
            //strcpy(curword, NEAT::getUnit(curline, curwordnum++, delimiters));
            ss >> curword;
            ...
            //cout<<endl;
        } else if (strcmp(curword, "trait") == 0) {                                        //Read in a trait
            Trait *newtrait;
        ...
        } else if (strcmp(curword, "node") == 0) {                                        //Read in a node
            NNode *newnode;
            ...
            nodes.push_back(newnode);
        } else if (strcmp(curword, "gene") == 0) {                                            //Read in a Gene
            Gene *newgene;
...
bool Genome::mutate_add_node(std::vector<Innovation *> &innovs, int &curnode_id, double &curinnov) {
    ...
    while (!done) {
        ...
        if (theinnov == innovs.end()) {
            ...
            innovs.push_back(new Innovation(in_node->node_id, out_node->node_id, curinnov - 2.0, curinnov - 1.0,
    ...
    if (found) {
        ...
        while (!done) {
            ...
            if (theinnov == innovs.end()) {
                ...
                //Create new gene
                newgene = new Gene(((thetrait[(*theinnov)->new_traitnum])), (*theinnov)->new_weight, nodep1, nodep2,
                                   recurflag, (*theinnov)->innovation_num1, 0);
        ...
        if (!skip) {
            ...
            if ((((chosengene->lnk)->linktrait)) == 0) traitnum = (*(traits.begin()))->trait_id - 1;
                ...
                traitnum = (((chosengene->lnk)->linktrait)->trait_id) -
                           (*(traits.begin()))->trait_id;  //The subtracted number normalizes depending on whether traits start counting at 1 or 0
