...
bool Species::print_to_file(std::ofstream &outFile) {
    ...
    //Print a comment on the Species info
    outFile << std::endl << "/* Species #" << id << " : (Size " << organisms.size() << ") (AF " << ave_fitness
            << ") (Age " << age << ")  */" << std::endl << std::endl;

    //Show user what's going on
    std::cout << std::endl << "/* Species #" << id << " : (Size " << organisms.size() << ") (AF " << ave_fitness
              << ") (Age " << age << ")  */" << std::endl;
    ...
    for (curorg = organisms.begin(); curorg != organisms.end(); ++curorg) {
        ...
        //Put the fitness for each organism in a comment
        outFile << std::endl << "/* Organism #" << ((*curorg)->gnome)->genome_id << " Fitness: " << (*curorg)->fitness
...
bool Species::print_to_file(std::ostream &outFile) {
    ...
    for (curorg = organisms.begin(); curorg != organisms.end(); ++curorg) {
        ...
        sprintf(tempbuf2, "/* Organism #%d Fitness: %f Time: %d */\n", ((*curorg)->gnome)->genome_id,
                (*curorg)->fitness, (*curorg)->time_alive);
        outFile << tempbuf2;
